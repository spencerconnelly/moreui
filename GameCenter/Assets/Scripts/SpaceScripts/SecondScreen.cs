...
public class SecondScreen : MonoBehaviour {
	...
	void OnGUI()
	{
		if (GUI.Button (new Rect (Screen.width/2 + 50, Screen.height/2 - 50, 200, 35), "BEGIN")) {
			sources [0].Play ();
			StartCoroutine ("play"); //Starts the game
		}
		if (GUI.Button (new Rect (Screen.width/2 + 50, Screen.height/2 + 10, 200, 35), "GAME LEVELS")) {
			sources [0].Play ();
			StartCoroutine ("levels");
		}
		if (GUI.Button (new Rect (Screen.width/2 + 50, Screen.height/2 + 70, 200, 35), "CONFIGURATION")) {
			sources [0].Play ();
			StartCoroutine ("config"); //Loads the main game screen
		}
		if (GUI.Button (new Rect (Screen.width/2 + 50, Screen.height/2 + 130, 200, 35), "HISTORY")) {
			sources [0].Play ();
		}
	}
